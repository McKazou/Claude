---
name: synchronisation-automatique-hub-skills-projets
description: Maintenir ce dépôt (Skills/ + Fiches/) comme source unique à jour — pousser une fiche/skill créée ou modifiée ici vers les projets pertinents listés dans l'historique de Sync-Skills.ps1, et ramener systématiquement dans ce dépôt tout skill créé directement dans un autre projet. À utiliser après toute création/modification de fiche/skill dans ce repo, et dès qu'un skill apparaît dans le .claude/skills/ d'un autre projet.
---
...
- **Pull (projet → hub)** : dans tous les cas, dès qu'un skill est créé ou modifié directement dans le `.claude/skills/` d'un autre projet (pas depuis ce hub), le ramener ici — copier son dossier dans `Skills/<nom>/`, produire/compléter la fiche correspondante dans `Fiches/` (voir [convertir-une-fiche-en-skill.md](../../Fiches/convertir-une-fiche-en-skill.md), à l'envers), et l'ajouter au `SOMMAIRE.md`.
- Avant de ramener un skill créé dans un projet d'entreprise, appliquer `separer-info-entreprise-vs-personnel` : ne rapatrier que la méthode généralisable, jamais une information propriétaire.
...
Source : `Fiches/synchronisation-automatique-hub-skills-projets.md`
