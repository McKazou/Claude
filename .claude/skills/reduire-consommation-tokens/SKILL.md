---
name: reduire-consommation-tokens
description: Applique des leviers concrets pour limiter le volume de tokens envoyés et reçus par requête (clear/compact, outils chargés, lecture ciblée de fichiers, progressive disclosure, sous-agents pour l'exploration volumineuse). À utiliser quand une session devient lente, coûteuse, ou approche la limite de contexte, ou quand l'utilisateur demande explicitement de réduire la consommation de tokens.
---
...
**Comment** :
- Proposer `/clear` entre deux tâches indépendantes plutôt que de garder tout l'historique d'une tâche terminée ; proposer `/compact` quand l'historique doit être conservé mais est devenu verbeux.
...
**Limites** : ces leviers réduisent aussi la quantité d'information disponible pour le modèle — à doser selon la tâche ; ne pas sacrifier la justesse d'une réponse pour économiser des tokens.

Source : `Fiches/reduire-consommation-tokens.md`
